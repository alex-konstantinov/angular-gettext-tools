...
function mkAttrRegex(startDelim, endDelim, optionalAttrMarkerName) {
    var start = startDelim.replace(escapeRegex, '\\$&');
    var end = endDelim.replace(escapeRegex, '\\$&');
	var marker = optionalAttrMarkerName != null && optionalAttrMarkerName !== '' ? optionalAttrMarkerName : 'translate';
	...
	return new RegExp(start + '\\s*(\'|"|&quot;|&#39;)(.*?)\\1\\s*\\|\\s*' + marker + '\\s*(' + end + '|\\|)', 'g');
}
...
var Extractor = (function () {
    function Extractor(options) {
        ...
        this.strings = {};
        this.attrRegex = mkAttrRegex(this.options.startDelim, this.options.endDelim, this.options.attrMarkerName);
		this.noDelimRegex = mkAttrRegex('', '', this.options.attrMarkerName);
    }
    ...
    Extractor.prototype.extractJs = function (filename, src, lineNumber) {
        ...
        function isGetString(node) {
            return self.options.markerNames.indexOf("gettext") > -1 &&
				node !== null &&
                node.type === 'CallExpression' &&
                node.callee !== null &&
        ...
        var extractHtml = function (src, lineNumber) {
            ...
            $('*').each(function (index, n) {
                ...
                for (var attr in node.attr()) {
					if (self.options.attrMarkerName !== 'sTranslate' && (attr === 'translate' || attr === 'data-translate') ) {
    ...
    Extractor.prototype.parse = function (filename, content) {
		...
		gutil.log('Parsing file ' + filename);
